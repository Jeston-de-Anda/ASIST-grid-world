#!/usr/bin/env python
"""
Sinkhorn Implementations in Numpy: a remaster of sinkhorn_torch
...
def sinkhorn_numpy(mat,
    ...
    '''
    n, m = mat.shape
    row_sum = row_sum if row_sum is not None else np.ones(n, dtype=np.float64)
    col_sum = col_sum if col_sum is not None else np.ones(m, dtype=np.float64)
